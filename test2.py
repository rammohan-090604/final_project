...
def encrypted_binary_to_image(encrypted_file, encryption_key=123, output_image='reconstructed_image.png'):
    ...
    if os.path.exists(encrypted_file):
        ...
        print(f"Deleted intermediate file: {encrypted_file}")
"""
####################################################################################################################################################################
Encrypted Binary to Video and Video to Encrypted Binary functions
"""

def encrypted_binary_to_video(encrypted_file, output_video='output_video.mp4', width=320, height=240, fps=24):
...
if __name__ == "__main__":
    while True:
        input_Data = input(
            ...
            "2. Reconstruct image from Video\n"
            "3. Convert text to Video\n"
            "4. Convert Video to text\n"
        ...
        if input_Data == "1":
            ...
            encrypted_binary_to_video('encrypted_image.bin')

        elif input_Data == "2":
            video_to_encrypted_binary('output_video.mp4')
            encrypted_binary_to_image('encrypted_video.bin')
            
        elif input_Data == "3":
            text_file = input("Enter the path to the text file: ")
            text_to_encrypted_binary(text_file)
            encrypted_binary_to_video('encrypted_binary_text.bin')

        elif input_Data == "4":
            video_to_encrypted_binary('output_video.mp4')
            encrypted_binary_to_text("encrypted_video.bin")
